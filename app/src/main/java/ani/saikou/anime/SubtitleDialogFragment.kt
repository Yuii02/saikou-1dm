        ...
        override fun onBindViewHolder(holder: StreamViewHolder, position: Int) {
            ...
            if (position == 0) {
                ...
                binding.subtitleTitle.text = when (subtitles[position - 1].language) {
                    ...
                    "sv-SE" -> "[sv-SE] Swedish"
                    else -> if(subtitles[position - 1].language matches Regex("([a-z]{2})-([A-Z]{2})") || subtitles[position - 1].language matches Regex("([a-z]{2})-(\\d+)")) "[${subtitles[position - 1].language}]" else subtitles[position - 1].language
                }
                model.getMedia().observe(viewLifecycleOwner) { media ->
                    val mediaID: Int = media.id
                    val selSubs: String? = loadData("subLang_${mediaID}", activity)
